import { Truck, Pencil, Save, X, Package, Send, MapPin, Phone, User } from 'lucide-react';
...
export function OrdenEnvioCard({ 
  ...
  // Determinar si se puede generar recogida (orden nueva sin código entrada)
  const puedeGenerarRecogida = isAdmin && !orden.codigo_recogida_entrada;
  
  // Determinar si se puede generar envío (orden validada/reparada sin código salida)
  ...
  // Datos de recogida/envío guardados
      ...
      <div className="mt-2 p-2 bg-slate-50 rounded text-xs space-y-1">
        ...
        {datos.nombre && (
          <p className="flex items-center gap-1">
            <User className="w-3 h-3" /> {datos.nombre}
          </p>
        )}
        {datos.direccion && (
          <p className="flex items-center gap-1">
            ...
            {datos.puerta && ` ${datos.puerta}`}
            {datos.codigo_postal && `, ${datos.codigo_postal}`}
            {datos.ciudad && ` ${datos.ciudad}`}
          </p>
        )}
        {datos.telefono && (
          <p className="flex items-center gap-1">
            <Phone className="w-3 h-3" /> {datos.telefono}
          </p>
        )}
        {datos.observaciones && (
          <p className="text-muted-foreground italic">"{datos.observaciones}"</p>
        )}
      </div>
    );
  };
        ...
        <CardTitle className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            {isAdmin && !editingEnvio && (
              <Button variant="ghost" size="sm" onClick={onEditEnvio} title="Editar datos">
        ...
        {editingEnvio ? (
          <div className="space-y-3">
            <div>
              ...
              <Input
                value={envioData.numero_autorizacion}
                onChange={(e) => setEnvioData(prev => ({ ...prev, numero_autorizacion: e.target.value }))}
            ...
            <div>
              ...
              <Input
                value={envioData.agencia_envio}
                onChange={(e) => setEnvioData(prev => ({ ...prev, agencia_envio: e.target.value }))}
            ...
            <div>
              ...
              <Input
                value={envioData.codigo_recogida_entrada}
                onChange={(e) => setEnvioData(prev => ({ ...prev, codigo_recogida_entrada: e.target.value }))}
            ...
            <div>
              ...
              <Input
                value={envioData.codigo_recogida_salida}
                onChange={(e) => setEnvioData(prev => ({ ...prev, codigo_recogida_salida: e.target.value }))}
                placeholder="Código salida"
                className="h-8 font-mono"
              />
            </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {puedeGenerarRecogida && onGenerarRecogida && (
                    <Button 
                      ...
                      data-testid="btn-generar-recogida"
                    >
